...
void PlayerControlUI::Update(const Vector2& playerScreenPos){
	...
	easeT = std::clamp(easeT, 0.f, totalEaseTime);
	curEaseScale = 1.f + (EaseOutQuad(easeT / totalEaseTime) * minEaseScale);
	spaceButton_->SetScale({ 0.3f * curEaseScale, 0.3f * curEaseScale });
...
void PlayerControlUI::Debug_Gui(){
	ImGui::Begin("debug");
	ImGui::Text("curEaseScale=%f", curEaseScale);
	ImGui::End();
