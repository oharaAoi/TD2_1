...
Texture2D<float4> gLightTexture : register(t1);
...
PixelShaderOutput main(VertexShaderOutput input) {
	...
	float2 scaledUV = transformedUV.xy * tilingFactor;
	float3 lightTextureColor = gLightTexture.Sample(gSampler, scaledUV).xyz * 2.0 - 1.0;
	
	float luminance = 0.299 * lightTextureColor.r + 0.587 * lightTextureColor.g + 0.114 * lightTextureColor.b;
